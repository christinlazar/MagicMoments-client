        ...
        const location = useLocation()
            ...
            useEffect(()=>{
                ...
                if(vendorData){
                    ...
                    setPhoneNumber(vendorData.data.data.phoneNumber)
                    setStartingPrice(vendorData.data.data.startingPrice)
                }
                }
                getvendorData()
            },[])
        
        const handleSaveEdits = async (e:React.FormEvent<HTMLFormElement>) =>{
            e.preventDefault()
            try {
                const formData = {
                    description,
                ...
                console.log("fmdata",formData)
                const response = await EditStoreDetails(formData)
                if(response?.data.success){
                    toast.success("Detials has been added successfully")
                    setTimeout(()=>{
                    navigate('/vendor/vendorStore')
                    },3000)
                }else{
            ...
            <div className="flex ps-4 md:ps-12">
              ...
              <div className="flex-grow ms-2 md:ms-6 mt-10">
                ...
                <div className='h-20 ms-2 mt-4 w-full max-w-xs'>
                  ...
                  <div>
                    <input className='text-sm ms-2 focus:outline-none placeholder:text-sm' placeholder={vendorData?.companyName} />
                  </div>
                </div>
                <div className='me-2'>
                ...
                <div>
                  {/* {
                    vendorData && vendorData?.description && vendorData.phoneNumber && vendorData.startingPrice ? ( */}
                      <form onSubmit={handleSaveEdits} method='POST'>
                        ...
                        <div className='flex flex-col sm:flex-row justify-start ms-2 pt-2 '>
                          <div className='flex flex-col mx-1'>
                            ...
                            <input onChange={(e)=>setPhoneNumber(e.target.value)} value={phoneNumber}  className='placeholder:text-sm w-full max-w-xs h-10 text-sm focus:outline-none border border-cyan-950 rounded ms-2 text-center' />
                          </div>
                          <div className='flex flex-col mx-1'>
                            <label className='text text-sm mt-3 ms-3 text-cyan-950 font-bold' htmlFor='name'>Starting Price</label>
                            <input onChange={(e)=>setStartingPrice(e.target.value)} value={startingPrice}  className='placeholder:text-sm w-full max-w-xs h-10 text-sm focus:outline-none border border-cyan-950 rounded ms-2 text-center' />
                          </div>
                          <div className='flex flex-col mx-1'>
                            <button type='submit'  className="px-10 ms-4  mt-6 py-3 h-12 rounded-full cursor-pointer text-white border-0 bg-cyan-950 shadow-md tracking-wider uppercase  transition-all duration-500 ease-linear hover:tracking-widest hover:bg-cyan-950 hover:text-white hover:shadow-lg hover:shadow-[rgba(20,24,54,0.93)] active:translate-y-2 active:tracking-widest text-xs active:bg-cyan-950 active:text-white active:shadow-none active:transition-none">Save edits</button>
                          </div>
                        </div>
                      </form>
                    {/* ) : ( */}
